...
export const QuayOrganization = ({ initialValue, value, error, setValue, setError }: Props) => {
  ...
  useEffect(() => {
    ...
    if (!value || value.trim() === '') {
      setError('Quay.io organization is required');
    } else {
  ...
  return (
    <Block
      ...
      description="Enter the Quay.io organization name. Data will be pulled from OCI artifacts (containers/images) in this Quay.io organization."
      required
    >
      <Input
        style={{ width: 386 }}
        placeholder="konflux-ci"
        value={value}
