...
type githubSnowflakePrivateKeyPlain struct {
	ID         uint64 `gorm:"primaryKey"`
	PrivateKey string `gorm:"column:private_key"`
}

func (githubSnowflakePrivateKeyPlain) TableName() string {
...
func (*encryptPrivateKey) Up(basicRes context.BasicRes) errors.Error {
	...
	if encKey == "" {
		return errors.BadInput.New("ENCRYPTION_SECRET is required to encrypt github_snowflake private keys")
	}

	cursor, err := db.Cursor(dal.From(&githubSnowflakePrivateKeyPlain{}))
	if err != nil {
		return err
	}
	defer cursor.Close()
	...
	for cursor.Next() {
		row := &githubSnowflakePrivateKeyPlain{}
...
func (*encryptPrivateKey) Name() string {
	return "encrypt github_snowflake connection private keys"
}
