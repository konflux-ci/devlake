...
// Productivity handles POST /api/metrics/pr/productivity.
func Productivity(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Printf("pr/productivity: %v", err)
			http.Error(w, "query error", http.StatusInternalServerError)
			return
		}
		api.WriteJSON(w, tpr.BuildProductivity(prs))
	}
}
