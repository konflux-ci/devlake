...
// Stages handles POST /api/metrics/pr/stages.
func Stages(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		stagePRs, err := qpr.StagesPRs(r.Context(), db, toParams(p))
		if err != nil {
			log.Printf("pr/stages: %v", err)
			http.Error(w, "query error", http.StatusInternalServerError)
			return
		}
		api.WriteJSON(w, tpr.BuildStages(stagePRs))
	}
}
